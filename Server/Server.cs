...
namespace DNSServer
{
    ...
    class QueryExecutor
    {
         ...
         */
        public QueryResult Execute(Domain domain, ResourceRecordType rtype, bool recursive)
        {
            ...
            {
                var records = zone.Query(domain, rtype).ToList();

                // It is possible that there is a CNAME that we should be aware of - in that case, check
                // it and see if we can find one.
                if (records.Count == 0)
                {
                    var cname_records = zone.Query(domain, ResourceRecordType.CANONICAL_NAME).ToArray();
                    ...
                    if (cname_records.Length != 0)
                    {
                        ...
                        var alias_results = Execute(alias, rtype, recursive);
            ...
            else
            {
                ...
                if (owning_subzone != null)
                {
                    ...
                    var subzone_nameservers = zone.Query(owning_subzone, ResourceRecordType.NAME_SERVER);
                    var subzone_nameserver_addr_records = subzone_nameservers.SelectMany(
                                                              ns => zone.Query(ns.Name, ResourceRecordType.HOST_ADDRESS)
                                                          );
                    ...
                    try
                    {
                        var info = resolver.Resolve(domain, ResourceRecordType.HOST_ADDRESS, subzone_nameserver_addrs);
                ...
                else if (recursive)
                {
                    ...
                    try
                    {
                        response = resolver.Resolve(domain, rtype, zone.Relays);
                    }
                    catch (ResolverException err)
                    {
    ...
    class UDPServer
    {
        ...
        private void RespondToClient(byte[] buffer, EndPoint peer)
        {
            ...
            try
            {
                ...
                outbound.IsQuery = false;
                outbound.QueryType = inbound.QueryType;
                outbound.WasTruncated = false;
                outbound.RecursiveRequest = inbound.RecursiveRequest;
                ...
                if (inbound.QueryType == QueryType.UNSUPPORTED)
                {
                    ...
                    outbound.ResponseType = ResponseType.NOT_IMPLEMENTED;
                    outbound.Questions = new DNSQuestion[] { };
                    outbound.Answers = new DNSRecord[] { };
                    outbound.AuthoritativeAnswers = new DNSRecord[] { };
                    outbound.AdditionalRecords = new DNSRecord[] { };
                }
                ...
                {
                    outbound.RecursiveResponse = outbound.RecursiveRequest;
                    ...
                    else
                    {
                        try
                        {
                            var answer = query_exec.Execute(question.Name, question.QueryType, inbound.RecursiveRequest);

                            if (answer.FoundAnswer)
                            {
                                logger.Trace("{0}: Resolver found answer for {1}", peer, question);
                                outbound.ResponseType = ResponseType.NO_ERROR;
                                outbound.IsAuthority = outbound.IsAuthority && answer.IsAuthority;
                                outbound.Questions = inbound.Questions;
                                outbound.Answers = answer.Answers.ToArray();
                                outbound.AdditionalRecords = answer.Additional.ToArray();
                                outbound.AuthoritativeAnswers = answer.Authority.ToArray();
                            }
                            else
                            {
                                logger.Trace("{0}: Could not find the name {1}", peer, question.Name);
                                outbound.ResponseType = ResponseType.NAME_ERROR;
                                outbound.IsAuthority = outbound.IsAuthority && answer.IsAuthority;
                                outbound.Questions = new DNSQuestion[] { };
                                outbound.Answers = new DNSRecord[] { };
                                outbound.AuthoritativeAnswers = new DNSRecord[] { };
                        ...
                        catch (Exception err)
                        {
                            ...
                            outbound.ResponseType = ResponseType.SERVER_FAILURE;
                            outbound.IsAuthority = false;
                            outbound.Questions = inbound.Questions;
                            outbound.Answers = new DNSRecord[] { };
                            outbound.AdditionalRecords = new DNSRecord[] { };
                            outbound.AuthoritativeAnswers = new DNSRecord[] { };
            ...
            catch (InvalidDataException err)
            {
                ...
                outbound.Id = in_id;
                outbound.IsQuery = false;
                outbound.QueryType = QueryType.STANDARD_QUERY;
                outbound.IsAuthority = false;
                outbound.WasTruncated = false;
                outbound.RecursiveRequest = false;
                outbound.RecursiveResponse = false;
                outbound.ResponseType = ResponseType.FORMAT_ERROR;
                outbound.Questions = new DNSQuestion[] { };
                outbound.Answers = new DNSRecord[] { };
                outbound.AuthoritativeAnswers = new DNSRecord[] { };
                outbound.AdditionalRecords = new DNSRecord[] { };
    ...
    public class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            ...
            var resolver = new StubResolver(cache);
